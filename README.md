...
このリポジトリは生成 AI 開発を前提にモダンな環境構築のテンプレートを提供するものです。例として、「3 の倍数と'3'がつく数字の時だけ Aho になる Transformer」を題材に、「2 の倍数と'2'がつく数字の時だけ Aho になる Transformer」、「5 の倍数と'5'がつく数字の時だけ Aho になる Transformer」、そして、「7 の倍数と'7'がつく数字の時だけ Aho になる Transformer」を、学習データの切り替えだけで学習するパイプラインを構築しました。ただし、2・3・5 では正解率は 100%であるのに対し、7 では正解率は 95.47%にとどまっています。ちなみに、2・3・5・7 のいずれの場合も学習データは 5 万件（うち、その 10%をバリデーションデータとして使う）、テストデータは学習データと完全に disjoint な数字で 5 万件使っています。このうち、学習とテストの両方で、「倍数か文字を含む」という条件に該当する数字はおおよそ 40% ～ 60%です。
...
ちなみに、なぜ 7 の場合は 100%の精度にならないのか？　というと、10 進表記に対する「倍数判定」の難易度のが数字 k で全然違うことが考えられます。k=2,5 は "末尾 1 桁" で決まる局所ルールの学習です。2 の倍数は末尾が 0,2,4,6,8 で、5 の倍数は末尾が 0,5 です。つまり Transformer にとっては最後のトークンだけ見れば良いレベルの局所パターンで、学習が極端に簡単です。また、k=3 は "各桁の和" で決まり、全桁を見るが、重みが全部同じケースに該当するので、位置（何桁目か）に依存しません。一方で、k=7 は "位置依存の重み付き和" になりやすく、右端基準の位相合わせが必要です。これを 100%の精度にするためには、存在しない桁の部分を PAD で埋めることや、数字を逆順にするなどの処理を入れることで対応できる可能性があると思います。
